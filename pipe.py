...
class Pipe(pygame.sprite.Group):
    def __init__(self, start, end, grid):
        ...
        end_square = grid[end]
        self.add(end_square)
        #self.add_square(self.end, grid)#TODO: don't use add_square for the initialisation of the end square
    ...
    def update(self, game_window, grid):
        ...
        if key_state[pygame.K_j]:
            ...
            new_x = self.front[0]
            if grid[(new_x, new_y)].is_occupied and grid[(new_x, new_y)] is not self.end:
                print('Square already occupied, choose another path.')
                time.sleep(5)
            else:
